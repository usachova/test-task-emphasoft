...
class BookingAPIView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        if Booking.objects.filter(room=request.POST.get('room')).filter(
                Q(date_start__lte=request.POST.get('date_finish')) |
...
class RoomAPISearch(APIView):
    ...
    def get(self, request):
        date_start = datetime.strptime(
            self.request.GET.get('date_start'), '%Y-%m-%d')
        date_finish = datetime.strptime(
            self.request.GET.get('date_finish'), '%Y-%m-%d')
        rooms = Room.objects.all()
        queryset = []
